class OPTCBioMecSubArmor extends X2DownloadableContentInfo config(BioMecSubArmorCompatibleClasses);

var config array<name> CompatibleClasses;

static function bool UnitIsCompatible(const XComGameState_Unit UnitState)
{
	return default.CompatibleClasses.Find(UnitState.GetSoldierClassTemplateName()) != INDEX_NONE;
}
    ...
    optional XComGameState_Item ItemState) // Item State of the Item we're trying to equip
{
	...
	if(!UnitIsCompatible(UnitState) &&	//	equip something on a non-SPARK, non-MEC Trooper class
		Slot == eInvSlot_Utility || Slot == eInvSlot_Vest)	// into the utility or vest slot
	{
		if (ItemTemplate.DataName == 'BioMecSubArmor' || ItemTemplate.DataName == 'ThermalSubArmor') // Bio MEC Sub-Armor and Thermal Sub-Armor only in this case
		{
